...
//      setTime(utc);      // Set system clock to utc time (not time zone compensated)
      utc = epoch;          //keep in syn with previous NTP scripts (not actually required)
      eastern = ausET.toLocal(utc);
      setTime(eastern);
